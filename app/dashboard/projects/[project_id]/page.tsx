  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4">
        <div className="flex items-start gap-3">
          <Button
            ...
            size="sm"
            onClick={() => router.back()}
            className="shrink-0 hover:bg-muted mt-1"
          >
            <ArrowLeft className="w-4 h-4 mr-2" />
            Back
          </Button>
          <div className="flex-1 min-w-0 space-y-1">
            ...
            <h1 className="display text-3xl sm:text-4xl text-foreground truncate">
              {project.name}
            </h1>
            <p className="text-sm text-muted-foreground">
              App ID: <span className="mono text-xs">{project.app_id}</span>
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card className="rebo-card hover:shadow-md transition-all duration-300 hover:-translate-y-0.5">
          <CardContent className="p-5">
            <div className="flex items-center justify-between">
              <div className="flex-1 min-w-0">
                <p className="text-xs sm:text-sm font-medium text-muted-foreground mb-1">Total Revenue</p>
                <p className="mono text-xl sm:text-2xl font-bold text-foreground">
      ...
      <Tabs defaultValue="overview" className="space-y-6">
        ...
        <TabsContent value="overview" className="space-y-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {/* Basic Information */}
            <Card className="rebo-card hover:shadow-md transition-all duration-300">
                  ...
                  <div className="space-y-2">
                    ...
                    {isEditing ? (
                      <LogoUrlField
                        ...
                        onChange={(value) => setEditData(prev => ({ ...prev, custom_logo: value }))}
                      />
                    ) : (
                ...
                <CardContent className="space-y-6 pt-6">
                  <div className="space-y-3">
                    ...
                    {isEditing ? (
                      <div className="flex items-center justify-between p-4 bg-muted rounded-lg">
                        <span className="text-sm text-muted-foreground">
                          Display the currency value to users in reward experiences
                        </span>
                        <Switch
              ...
              <CardContent className="space-y-6 pt-6">
                <div className="space-y-2">
                  ...
                  <div className="p-4 bg-muted/50 rounded-lg border border-border">
                    <p className="mono text-2xl sm:text-3xl font-bold text-foreground">
                      ${project.revenue.toFixed(2)}
                    </p>
              ...
              <CardHeader className="pb-4 border-b border-border/50">
                ...
                <CardContent className="space-y-6 pt-6">
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-muted-foreground">
                      URL to notify when users complete verified reward events
                    </p>
                  </div>
...
// Shows a live preview of the pasted logo URL instead of the raw text, with an
// X to clear it. Falls back to the plain URL input if the image can't load —
// broken links still need to be visible and editable.
function LogoUrlField({ value, onChange }: { value: string; onChange: (value: string) => void }) {
  ...
  if (value && status === 'loaded') {
    return (
      <div className="flex items-center gap-3">
        <div className="flex h-10 items-center rounded-lg border border-border bg-muted px-3">
          <Image src={value} alt="Logo preview" width={140} height={24} unoptimized className="h-6 w-auto max-w-[140px] object-contain" />
        </div>
    ...
    <div className="space-y-1.5">
      <div className="flex items-center gap-3">
        <Input
          ...
          className="h-10"
        />
        {value && status === 'loading' && <Loader2 className="w-4 h-4 shrink-0 animate-spin text-muted-foreground" />}
        {value && (
          <Button variant="outline" size="sm" onClick={() => onChange('')} className="shrink-0">
            <X className="w-4 h-4" />
          </Button>
