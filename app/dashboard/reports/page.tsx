...
import { Users, DollarSign, Target, Zap, Loader2, AlertCircle, RefreshCw, BarChart3, LineChart, PieChart, Activity } from 'lucide-react'
...
export default function ReportsPage() {
  ...
  useEffect(() => {
    if (user && projects && projects.length > 0 && dateRange?.from && dateRange?.to) {
      fetchFilteredStats()
    }
    // fetchFilteredStats is defined below and stable within the render
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [user, projects, dateRange])
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card className="bg-card border-border hover:shadow-lg transition-shadow">
          <CardHeader className="pb-4 border-b border-border/50">
            <CardTitle className="text-base sm:text-lg font-semibold">Reward Events</CardTitle>
          </CardHeader>
          <CardContent>
            {userGrowthData.length > 0 && userGrowthData.some((item: any) => item.users > 0) ? (
              ...
              <div className="h-[300px] flex flex-col items-center justify-center text-muted-foreground space-y-3">
                ...
                <p className="text-sm font-medium">No reward event data available</p>
      ...
      <Card className="bg-card border-border hover:shadow-lg transition-shadow">
        <CardHeader className="pb-4 border-b border-border/50">
          <CardTitle className="text-base sm:text-lg font-semibold">Revenue by Event Type</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div>
