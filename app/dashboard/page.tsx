...
export default function DashboardPage() {
  ...
  const metrics = stats?.stats?.slice(0, 3).map((stat, index) => {
    ...
    { title: 'Total Revenue', value: '$0', icon: DollarSign, iconColor: 'text-green-500', bgColor: 'bg-green-500/10', change: 'Current total', changeColor: 'text-muted-foreground', ChangeIcon: TrendingUp, trend: 'up' },
    { title: 'Active Projects', value: '0', icon: Users, iconColor: 'text-blue-500', bgColor: 'bg-blue-500/10', change: 'Current total', changeColor: 'text-muted-foreground', ChangeIcon: TrendingUp, trend: 'up' },
    { title: 'Total Rewards', value: '0', icon: TrendingUp, iconColor: 'text-purple-500', bgColor: 'bg-purple-500/10', change: 'Current total', changeColor: 'text-muted-foreground', ChangeIcon: TrendingUp, trend: 'up' },
  ]
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
        {metrics.map((metric, index) => {
          return (
            <Card key={metric.title} className="rebo-card border-border hover:shadow-md transition-all duration-300">
              <CardContent className="p-5 sm:p-6">
                <div className="flex items-start justify-between">
                  <div className="flex-1 min-w-0">
                    <p className="text-xs sm:text-sm font-medium text-muted-foreground mb-2">{metric.title}</p>
                    <p className="mono text-xl sm:text-2xl font-semibold text-foreground mb-3">{metric.value}</p>
                    <div className="text-xs sm:text-sm text-muted-foreground flex items-center gap-1.5">
                      <span>{metric.change}</span>
                    </div>
                  </div>
                  <div className="relative shrink-0">
                    <div className="p-2.5 sm:p-3 rounded-lg bg-secondary border border-border">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card className="rebo-card">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-4">
            <CardTitle className="text-lg font-semibold text-foreground">Reward Events</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="mb-4">
              <p className="mono text-2xl font-semibold text-foreground">
              ...
              <p className="text-sm text-muted-foreground">Total verified reward events</p>
            </div>
            ...
            <Chart data={userGrowthData} type="bar" height={250} />
              ...
              <div className="h-[250px] flex flex-col items-center justify-center text-muted-foreground space-y-3">
                ...
                <p className="text-sm font-medium">No reward data yet</p>
                <p className="text-xs text-muted-foreground/80">Verified reward events will appear here once completed</p>
              </div>
            )}
          </CardContent>
        </Card>
